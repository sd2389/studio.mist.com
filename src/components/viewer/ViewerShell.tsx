...
import { EmbedChrome } from "@/components/viewer/EmbedChrome";
import { StudioSidebar } from "@/components/viewer/StudioSidebar";
import { StudioTopBar } from "@/components/viewer/StudioTopBar";
import { ZoomControls } from "@/components/viewer/ZoomControls";
import { resolveModelUrl } from "@/lib/model-url";
import { getSceneByViewerId, updateSceneByViewerId } from "@/lib/api/scenes";
import { fetchGemoraCatalog, type GemoraCatalogPayload } from "@/lib/gemora-catalog";
...
export function ViewerShell({ modelId, variant }: ViewerShellProps) {
  ...
  useEffect(() => {
    ...
    void getSceneByViewerId(modelId)
      .then((scene) => {
        ...
        replaceSlotSelections((scene.slot_selections ?? {}) as Record<string, MaterialPresetId>);
  ...
  }, [modelId, replaceSceneSettings, replaceSlotSelections, setLighting, setPreset]);
  ...
  if (variant === "embed") {
    return (
      <div className="relative h-[100dvh] w-full overflow-hidden bg-white">
        <EmbedChrome modelId={modelId} />
        <div className="absolute inset-0 top-12 min-h-0">
          <ViewerCanvas
            modelUrl={modelUrl}
            preset={preset}
