...
import { getJewelryById, type JewelryId } from "@/lib/jewelry/assembly";
        ...
        <div className="flex min-h-0 min-w-0 flex-1 flex-col bg-white">
          <header className="relative flex shrink-0 items-center justify-between gap-3 border-b border-border bg-card px-3 py-2.5 sm:px-4">
            <div className="min-w-0 flex-1">
              <p className="truncate text-base font-semibold text-foreground sm:text-lg">{piece.label}</p>
              <p className="truncate text-[10px] text-muted-foreground sm:text-xs">
                Demo · {preset} · {lighting}
              </p>
            </div>
            <Link
              href="/gallery"
